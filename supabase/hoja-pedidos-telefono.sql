-- ============================================================================
-- RepartoPro — Teléfono del cliente en Hoja de Pedidos
-- ============================================================================

ALTER TABLE public.order_sheet_rows ADD COLUMN IF NOT EXISTS phone text NOT NULL DEFAULT '';
...
BEGIN
  ...
  INSERT INTO public.order_sheet_rows (company_id, employee, date, address, turno, phone)
  VALUES (v_company, p_employee, COALESCE(p_date, ''), COALESCE(p_address, ''), COALESCE(p_turno, 1), COALESCE(p_phone, ''))
...
GRANT EXECUTE ON FUNCTION public.create_order_row(text, text, text, jsonb, int, text) TO authenticated;
